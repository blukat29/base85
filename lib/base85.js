...
function encodeBufferIPv6(buffer)
{
  ...
  return encodeBignumIPv6(new BigNumber(hex));
}
...
function encodeStringIPv6(string)
{
  ...
  var hex = '0x' + addr.parsedAddress.map(function(el) {
    return pad(4, el);
  }).join('');
  return encodeBignumIPv6(new BigNumber(hex));
}
...
function decodeStringIPv6(string)
{
  ...
  try {
    var result = string.split('').reduceRight(function(memo, el) {
      var idx = dectable[el.charCodeAt(0)];
      if (idx === undefined) {
        throw new Error('Invalid character');
      }
      var num = new BigNumber(idx);
      var fact = new BigNumber(85).pow(i++);
      var contrib = num.times(fact);
      return memo.plus(contrib);
    }, new BigNumber(0));

    var bigint = new BigInteger(result.toFixed());
    return Address6.fromBigInteger(bigint).correctForm();
  } catch(e) {
    return false;
  }
}
